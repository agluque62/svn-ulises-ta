...
namespace HMI.Infrastructure.Interface
{
	...
	{
		[EventPublication(EventTopicNames.ActiveViewChanging, PublicationScope.Global)]
		public event EventHandler<EventArgs<string>> ActiveViewChanging;
		...
		protected virtual TView ShowViewInWorkspace<TView>(string workspaceName)
		{
			...
			General.SafeLaunchEvent(ActiveViewChanging, this, new EventArgs<string>(""));
			WorkItem.Workspaces[workspaceName].Show(view);

			return view;
		}

		protected virtual TView ShowViewInWorkspace<TView>(string workspaceName, ISmartPartInfo info)
		{
			TView view = WorkItem.SmartParts.AddNew<TView>();
		...
		protected virtual TView ShowViewInWorkspace<TView>(string viewId, string workspaceName)
		{
			...
			General.SafeLaunchEvent(ActiveViewChanging, this, new EventArgs<string>(viewId));
			WorkItem.Workspaces[workspaceName].Show(view);

			return view;
		}

		protected virtual TView ShowViewInWorkspace<TView>(string viewId, string workspaceName, ISmartPartInfo info)
		{
			TView view = default(TView);
			if (WorkItem.SmartParts.Contains(viewId))
			{
		...
		protected virtual void HideViewInWorkspace<TView>(string viewId, string workspaceName)
		{
			if (WorkItem.SmartParts.Contains(viewId))
			{
				TView view = WorkItem.SmartParts.Get<TView>(viewId);
				WorkItem.Workspaces[workspaceName].Hide(view);
			}
		}
	}
}
