...
namespace HMI.CD40.Module.BusinessEntities
{
...
#endif
	{
		...
		{
			//lalm 220603 Devuelvo mcodgo decline con reason_code
			string reason_text = answer.reason_text;
			int code = answer.reason_code;
			foreach (LcPosition lc in _LcPositions)
			{
				answer.Value = lc.HandleIncomingCall(call, info, inInfo, out reason_text);
				if (reason_text != "")
				{
					answer.reason_text = reason_text;
					// podria poner cuaquier codigo y habria que ponderarlo para poner el mas importante
					answer.reason_code = SipAgent.SIP_DECLINE;
				}
