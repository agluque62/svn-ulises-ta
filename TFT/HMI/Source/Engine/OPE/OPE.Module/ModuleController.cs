...
using HMI.OPE.Module.Services;
using HMI.OPE.Module.Constants;

namespace HMI.OPE.Module
{
	public class ModuleController : WorkItemController
	{
		public override void Run()
		{
			...
			WorkItem.RootWorkItem.RunStarted += delegate(object sender, EventArgs e) 
			{
				((OpeCmdManagerService)WorkItem.RootWorkItem.Services.Get<IEngineCmdManagerService>()).Run(); 
			};
			WorkItem.RootWorkItem.Terminated += delegate(object sender, EventArgs e) 
			{
				((OpeCmdManagerService)WorkItem.RootWorkItem.Services.Get<IEngineCmdManagerService>()).Stop();
			};
		}
		...
		private void AddServices()
		{
			...
			WorkItem.RootWorkItem.Services.AddNew<OpeCmdManagerService, IEngineCmdManagerService>();
		}
		...
		private void ExtendMenu()
		{
			...
			ToolStripMenuItem opeInfo = new ToolStripMenuItem("SCV Info");
			opeInfo.Name = "EngineInfo";
			opeInfo.Margin = new Padding(opeInfo.Margin.Left, opeInfo.Margin.Top + 5, opeInfo.Margin.Right, opeInfo.Margin.Bottom + 5);
			opeInfo.Click += delegate(object sender, EventArgs e)
			{
				WorkItem.RootWorkItem.Services.Get<IEngineCmdManagerService>().GetEngineInfo();
			};

			WorkItem.RootWorkItem.UIExtensionSites[UIExtensionSiteNames.MainContextMenu].Add(opeInfo);
		}
