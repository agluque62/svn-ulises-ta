...
namespace HMI.CD40.Snmp
{
	static class SnmpAgent
	{
		...
		public static void Init(string ip, string trapMcastIp = null, int port = 161, int trap = 162)
		{
			...
			_engine.ExceptionRaised += (sender, e) => _logger.ErrorException("ERROR Snmp", e.Exception);
		...
		public static void GetValueAsync(IPEndPoint ep, string oid, Action<ISnmpData> handler, int timeout = 2000)
		{
		...
		public static void SetValueAsync(IPEndPoint ep, string oid, ISnmpData data, Action<ISnmpData> handler, int timeout = 4000)
		{
			ThreadPool.QueueUserWorkItem(delegate
			{
		...
		public static void GetAsync(IPEndPoint ep, IList<Variable> vList, Action<IList<Variable>> handler, int timeout = 4000)
		{
			ThreadPool.QueueUserWorkItem(delegate
			{
				try
